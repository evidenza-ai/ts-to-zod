...
function buildZodPrimitiveInternal({
      ...
      for (const node of nodes) {
        if (!ts.isTypeLiteralNode(node) && !ts.isTypeReferenceNode(node)) {
          if (process.env.NODE_ENV !== "test") {
            console.warn(
              ` »   Warning: discriminated union member "${node.getText(sourceFile)}" is not a type reference or object literal`
            );
          }
        ...
        if (ts.isTypeLiteralNode(node)) {
          ...
          if (!hasDiscriminator) {
            if (process.env.NODE_ENV !== "test") {
              console.warn(
                ` »   Warning: discriminated union member "${node.getText(sourceFile)}" missing discriminator field "${jsDocTags.discriminator}"`
              );
            }
            isValidDiscriminatedUnion = false;
            break;
  ...
  if (ts.isFunctionTypeNode(typeNode)) {
    ...
    const inputProperty = f.createPropertyAssignment(
      ...
      f.createArrayLiteralExpression(argsArray, false)
    );
    ...
    typeNode.templateSpans.forEach((span) => {
      if (ts.isTypeReferenceNode(span.type)) {
        ...
        if (targetNode) {
              ...
              targetNode.members
                .map((i) => {
                  if (i.initializer) return extractLiteralValue(i.initializer);
                    ...
                    console.warn(
                      ` »   Warning: enum member without initializer '${targetNode.name.getText(sourceFile)}.${i.name.getText(sourceFile)}' is not supported.`
                    );
                    ignoreNode = true;
                  }
                  return "";
          ...
          console.warn(
            ` »   Warning: reference not found '${span.type.getText(sourceFile)}' in Template Literal.`
          );
          ignoreNode = true;
        }
        ...
        console.warn(
          ` »   Warning: node '${span.type.getText(sourceFile)}' not supported in Template Literal.`
        );
        ignoreNode = true;
      }
    });
...
function buildSchemaReference(
  { node, dependencies, sourceFile, getDependencyName },
  path = ""
) {
  ...
  if (indexTypeName === "-1" || indexTypeName === "number") {
    ...
    if (declaration && ts.isIndexedAccessTypeNode(node.objectType)) {
      ...
      if (member && ts.isPropertySignature(member) && member.type) {
        ...
        if (
          ts.isTypeReferenceNode(member.type) &&
          member.type.typeName.getText(sourceFile) === "Map"
        ) {
          return buildSchemaReference(
            {
              node: node.objectType,
...
function buildOmitPickObject(
    ...
    parameters = f.createObjectLiteralExpression(
      keys.types.map((type) => {
        if (!ts.isLiteralTypeNode(type)) {
          throw new Error(
            `${omitOrPickIdentifierName}<T, K> unknown syntax: (${ts.SyntaxKind[type.kind]} as K union part not supported)`
          );
        }
  ...
  if (!parameters) {
    throw new Error(
      `${omitOrPickIdentifierName}<T, K> unknown syntax: (${ts.SyntaxKind[keys.kind]} as K not supported)`
    );
  }
  return f.createCallExpression(
    f.createPropertyAccessExpression(
